...
本作业基于一个已经搭好的 2D 网格导航仿真器，你需要完成 **代价地图 / 全局规划 / 局部控制** 三大模块的核心算法，使机器人能够地图中自主导航到目标点。
...
- `--record` 参数会把仿真过程录成视频，方便你调参和最终提交。

控制方式：

- 左键单击地图：临时设置目标点（`debug` 模式，不计时）
- 右键单击地图：重置机器人和动态障碍物，回到 `timed` 模式，并重新计时
- ESC / 关闭窗口：退出

## 需要修改的目录与文件

需要修改的部分都以 `TODO` 注释标记，集中在 `navigation/nav/` 目录下的四个函数。

- [`compute_costmap`](../navigation/nav/costmap.py)
- [`update_local_costmap`](../navigation/nav/costmap.py)
- [`global_plan`](../navigation/nav/planner.py)
- [`local_plan`](../navigation/nav/controller.py)

你可以以任意方式实现这四个部分，无论是调库还是自己写算法，只要满足接口要求和验收标准即可。

可以调整 `main.py` 里调用这四个函数的传入参数或逻辑，但没有必要、也尽量不要这样做。但可能需要了解 `main.py` 的实现细节。

不要修改其他文件，如果确认是其他文件的问题，欢迎提 issue 说明。

下面的内容与文件中的英文注释基本相同，你可以直接参考文件中的注释来实现，也可以忽略这些注释，使用更好的方法。

### 1. `compute_costmap(static_map)`

**做什么**：把 0/1 的二值静态地图变成一个 `uint8` 的代价地图。障碍物本身是一个很大的值，障碍物附近一定半径内的自由格子有一个随距离衰减的"软代价"，更远的地方代价是 0。
...
**做什么**：在代价地图上跑路径搜索，返回 start 到 goal 的世界坐标路径点列表。

**思路提示：**

- 可以使用 8 连通性（上下左右四条线 + 4 条对角线）进行路径搜索。相邻单元格之间的步长成本应为 `dist + cell_cost`，其中 `dist` 对于直线移动为 1.0，对于对角线移动为 sqrt(2)。
...
- 可以使用更复杂的局部控制（如 Dynamic Window Approach），也可以尝试使用更复杂的模型预测方法（如 MPPI）。
...
├── configs/
│   └── cfg.yaml           # 任务配置（地图路径、机器人参数、任务目标等）
├── core/
│   ├── recorder.py        # 视频记录与回放
│   ├── renderer.py        # 基于Pygame 的视觉渲染引擎
...
最后提交作业时需要复原 [`cfg.yaml`](../navigation/configs/cfg.yaml) 中的默认配置，以保证评测环境的一致性。你可以根据这个配置中的参数进行一些针对性的算法优化。

### 任务终止条件

[`core/task.py`](../navigation/core/task.py) 的 `Task` 类维护两个模式：

- **`timed` 模式**：起始点和目标点都在 [`cfg.yaml`](../navigation/configs/cfg.yaml) 中配置，计时器从 0 开始递增。
- **`debug` 模式**：目标点是你左键点的位置，计时器冻结，不参与评测。

`timed` 模式下，**只有同时满足以下两个条件，计时器才会停止**：
边
1. 机器人到目标的距离 `< task.goal_threshold`
2. 最近 `task.stable_frames` 帧的位置坐标全部满足 `|x - x_0| < pos_eps` 且 `|y - y_0| < pos_eps`（即机器人真的停下来了，而不是高速冲过终点圈）
...
**2. 加速度限幅**：机器人不能瞬间从当前速度跳到目标速度。本帧速度增量被裁剪到 `max_accel * dt`
...
动态障碍物被绑定在一个矩形区域内做匀速运动，位置更新和碰撞逻辑与机器人相同，碰到界或者静态墙时把该轴速度反向（`vx = -vx` 或 `vy = -vy`）。

- 动态障碍物的数量、大小、矩形区域范围等参数在 [`cfg.yaml`](../navigation/configs/cfg.yaml) 中配置。
- 动态障碍物的初始速度方向是随机的，速度大小在 [`cfg.yaml`](../navigation/configs/cfg.yaml) 的 `world.dynamic_speed_range` 区间内均匀采样
